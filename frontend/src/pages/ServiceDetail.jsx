...
const ServiceDetail = () => {
  ...
  const fetchServiceHistory = useCallback(async () => {
    try {
      ...
      const historyData = await api.getServiceHistory(id, filterParams);
      setData(historyData);
      setError(null);
    } catch (err) {
  ...
  }, [id, filterParams, isInitialLoad]);

  useEffect(() => {
    fetchServiceHistory();
  }, [fetchServiceHistory]);

  // Reset initial load flag when service ID changes
  useEffect(() => {
  ...
  const { service, history, daily_stats, start_date, end_date } = data;
  ...
  // Get service name from service object (stable across filter changes)
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="mb-4 bg-slate-100 rounded-lg p-4 inline-block">
                <ServiceLogo serviceName={serviceName} size="xl" />
              </div>
